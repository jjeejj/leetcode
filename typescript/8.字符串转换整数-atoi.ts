...
class Automaton {
  sign: number = 1; // 符号位 正号/负号
  ans: number = 0; // 转换为结果的数字
  state: string = "start"; // 开始的状态
  ...
  // 确定下一个字符的状态
  private nextState(c: string): 0 | 1 | 2 | 3 {
    if (c === " ") {
      return 0;
    }
    if (c === "+" || c === "-") {
  ...
  // 处理每个字符的结果
  public handle(c: string) {
    this.state = this.table[this.state][this.nextState(c)];
    if ("in_number" === this.state) {
      this.ans =
        this.sign === 1
          ? this.ans > (Math.pow(2, 31) - 1 - <any>c) / 10
            ? Math.pow(2, 31) - 1
            : this.ans * 10 + (<any>c - 0)
          : this.ans > (Math.pow(2, 31) - <any>c) / 10
          ? -Math.pow(2, 31)
          : this.ans * 10 + (<any>c - 0);
...
function myAtoi1(s: string): number {
  ...
  if (s[idx] === "-") {
    ...
    idx++;
  } else if (!/[\d]/.test(s[idx])) {
    return 0;
  }
  let result: number = 0;
  while (idx < len && /[\d]/.test(s[idx])) {
    let digit = <any>s[idx] - 0;
    if (result > (Math.pow(2, 31) - 1 - digit) / 10) {
      return negative ? -Math.pow(2, 31) : Math.pow(2, 31) - 1;
    }
