/*
 * @lc app=leetcode.cn id=1921 lang=typescript
 *
 * [1921] 消灭怪物的最大数量
 *
 * https://leetcode-cn.com/problems/eliminate-maximum-number-of-monsters/description/
 *
 * algorithms
 * Medium (34.03%)
 * Likes:    16
...
function eliminateMaximum(dist: number[], speed: number[]): number {
  ...
  let times: number[] = [];
  for (let i = 0; i < n; i++) {
    times.push(Math.floor((dist[i] - 1) / speed[i]));
  }
  times.sort((a, b) => {
    return a - b;
  });
